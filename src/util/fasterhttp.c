...
__thread	char			g_init_httpstatus = 0 ;
__thread	struct HttpStatus	g_httpstatus_array[ 505 + 1 ] = { { 0 , NULL , NULL } } ;
...
static int CompressBuffer( struct HttpEnv *e , struct HttpBuffer *b , char *p_compress_algorithm , int compress_algorithm_len , int compress_algorithm )
{
	...
	p_CONTENT_LENGTH = strcasestr( b->base , HTTP_HEADER_CONTENT_LENGTH ) ;
...
static __thread struct HttpBuffer	__fasterhttp_status_code_httpbuf ;

static int FormatHttpResponseStartLineV( int status_code , struct HttpEnv *e , uint options, int fill_body_with_status_flag , char *format, va_list valist )
{
	...
	if( CheckHttpKeepAlive(e) )
	{
		if( options & HTTP_OPTIONS_FILL_CONNECTION_KEEPALIVE )
			nret = StrcatHttpBuffer( b , "Connection: keep-alive" HTTP_RETURN_NEWLINE ) ;
		else if( options & HTTP_OPTIONS_FILL_CONNECTION_CLOSE )
			nret = StrcatHttpBuffer( b , "Connection: close" HTTP_RETURN_NEWLINE ) ;
	}
	else if( e->headers.version == HTTP_VERSION_1_1_N )
	{
		nret = StrcatHttpBuffer( b , "Connection: close" HTTP_RETURN_NEWLINE ) ;
...
int FormatHttpResponseStartLine( int status_code , struct HttpEnv *e , int fill_body_with_status_flag , char *format , ... )
{
	...
	nret = FormatHttpResponseStartLineV( status_code, e, HTTP_OPTIONS_FILL_CONNECTION_KEEPALIVE, fill_body_with_status_flag, format, valist );
	va_end( valist );

	return nret;
}

int FormatHttpResponseStartLine2( int status_code , struct HttpEnv *e , uint options, int fill_body_with_status_flag , char *format , ... )
{
	va_list	valist ;
        int	nret = 0 ;
 	
	va_start( valist ,format );
	nret = FormatHttpResponseStartLineV( status_code, e, options, fill_body_with_status_flag, format, valist );
	va_end( valist );
...
static struct HttpHeader *QueryHttpHeader( struct HttpEnv *e , char *name )
{
	int			i ;
	...
	for( i = 0 , p_header = e->headers.header_array ; i < e->headers.header_array_size ; i++ , p_header++ )
	{
		...
		if( p_header->p_buffer && p_header->p_buffer->base && p_header->name_len == name_len && STRNICMP( p_header->p_buffer->base+p_header->name_offset , == , name , name_len ) )
		{
			return p_header;
		}
	}
